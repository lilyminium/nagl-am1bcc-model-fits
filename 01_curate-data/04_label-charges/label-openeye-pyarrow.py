...
def label_molecule(smiles: str, charge_method: str = "am1bccelf10") -> torch.tensor:
    from openff.toolkit import Molecule
    from openff.units import unit
    from openff.toolkit.utils import OpenEyeToolkitWrapper

    offmol = Molecule.from_mapped_smiles(smiles, allow_undefined_stereo=True)
    offmol.assign_partial_charges(charge_method, toolkit_registry=OpenEyeToolkitWrapper())
    ...
    return {
        ...
        f"{charge_method}_charges_openeye": charges,
    }


def batch_label(
    all_smiles: str,
    charge_method: str = "am1bccelf10"
):
    entries = []
    errors = []
    for smiles in tqdm.tqdm(all_smiles):
        try:
            entries.append(label_molecule(smiles, charge_method=charge_method))
...
@click.option(
    "--charge-method",
    help="The charge method to use.",
    type=click.Choice(["am1bccelf10", "am1bcc", "am1elf10"]),
    default="am1bccelf10",
    show_default=True,
)
...
def label_dataset(
    ...
    charge_method: typing.Literal["am1bccelf10", "am1bcc", "am1elf10"] = "am1bccelf10",
    ...
    with batch_distributed(
        ...
        futures = list(batcher(batch_label, charge_method=charge_method))
